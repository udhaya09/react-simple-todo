...
function App() {
  ...
  const updateToDo = (id, description, assigned) => {
    setTodos(prevTodos => {
    ...
    <div className="mt-5 container">
      <div className="card">
        ...
        <div className='card-body'>
          ...
          {
            showAddNew && <NewToDoForm addToDo={addToDo} updateToDo={updateToDo}  isEdit={isEdit} id={editRecordId}
            description={todos.find(todo => todo.id === editRecordId)?.description}
            assigned={todos.find(todo => todo.id === editRecordId)?.assigned}/>
          }
